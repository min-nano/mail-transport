...
iCloud の**受信トレイ**のメールを **SMTP を経由せず** Gmail に複製します。
iCloud からは **IMAP** で生のメッセージを取り出し、Gmail へは **Gmail API の
`users.messages.insert`** で直接書き込みます。
迷惑メールフォルダは移動対象に含めません (必要なら `ROUTES` で足せます)。
転送し終えたメールは **iCloud のゴミ箱へ移して**、iCloud の容量を空けます
(`TRASH_AFTER_FORWARD=false` で無効化できます)。
...
│             │ ◄── BODY.PEEK[] ─│  + SQLite (同期位置)   │                │             │
│  ゴミ箱     │ ◄── UID MOVE ────│  (転送が済んだメール)  │                │             │
└─────────────┘                  └──────────────────────┘                └─────────────┘
  迷惑メールは対象外 (iCloud 側に残す)
```
...
- IMAP は 1 接続につき 1 メールボックスしか選択できないため、
  `ROUTES` のメールボックスごとに別スレッド・別接続で待ち受けます
  (既定では受信トレイの 1 本だけです)。
...
転送するのは**受信トレイだけ**です。iCloud 側で迷惑メールと判定されたものは
Gmail に持ち込まず、iCloud の迷惑メールフォルダに残します。

- `source` — IMAP のメールボックス名、または `\Junk` のような
  **RFC 6154 の特殊用途フラグ**。フォルダ名はロケールによって `Junk` だったり
...
- **移動に失敗しても転送は成功扱い**です。Gmail 側には入っているので、失敗を理由に
  再送すると二重取り込みになります。警告ログと同期レポートの `trash_error` に
  残るので、メールは iCloud に残ったままになります。
- 移動先は `\Trash` (RFC 6154 の特殊用途フラグ) で探します。iCloud では
  「Deleted Messages」ですが、ロケールによって表示名が変わるためフラグで解決します。
  別のフォルダへ退避したい場合は `TRASH_MAILBOX` にメールボックス名を書きます。
...
2. **メールボックス解決** — `ROUTES` の `source` を実際のメールボックス名に変換
   (`\Junk` のような特殊用途フラグ指定にも対応)
3. **`SELECT`** — 本文の取得は常に `BODY.PEEK[]` なので既読にはならない。
   `TRASH_AFTER_FORWARD=false` のときは読み取り専用 (`EXAMINE`) で開く
...
- **受信後に iCloud 側で迷惑メールへ移されたメールは、すでに転送済みなら Gmail に
  残ります。** 同期は取り込み時点の状態を写す一方向のもので、移動は追随しません。
