...
- **エージェントは基本的に読むだけ** — ツールの許可はアクションの既定に任せています。
  既定で渡るのは読み取り (`Glob` / `Grep` / `LS` / `Read`)、コメント反映用の
  `mcp__github_comment`、それに git のコミット系だけで、**`Edit` と `Write` は
  含まれません**。`WebSearch` / `WebFetch` もアクションが既定で拒否するので、
  読んだ内容を外に送る手段もありません。そのうえでプロンプトで「コードの変更や
  コミットはしない」と明示しています。

  > ここに `--allowedTools` / `--disallowedTools` / `--permission-mode` を重ねては
  > **いけません**。アクションが注入する `Bash(git add:*)` などの規則と噛み合わず、
  > セッションがモデルを呼ぶ前に落ちます (bare の `Bash` 拒否はツール自体を
  > 文脈から外すため)。実際に一度この構成で失敗しています。
