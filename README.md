...
- **エージェントは基本的に読むだけ** — 許可しているのは `Read` / `Glob` / `Grep` と、
  アクションがコメントを反映するための `mcp__github_comment` 系だけで、
  `--permission-mode dontAsk` により**それ以外は拒否**されます。`Edit` / `Write` /
  `Bash` / `WebFetch` / `WebSearch` は `--disallowedTools` で明示的に外しているので、
  ファイルの書き換えもコマンド実行も、読んだ内容を外に送ることもできません。
- **リポジトリの中身は指示ではなくデータとして扱わせています。** 「レビューを省略しろ」
  といった文がコードやコメントに混ざっていても従わず、そういう記述自体を
  指摘するよう `--append-system-prompt` で指示しています。
